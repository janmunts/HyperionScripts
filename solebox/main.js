        ...
        if (result.initialized === true) {
            if (result.account.discordAuth === true) {
                if (result.active === true) {
                    console.log("Extension active, checking selected mode.");
                    if (result.websites.solebox.mode === "SAFE") {
                        ...
                        safe.urlCheck();
                    } else if (result.websites.solebox.mode === "REQUESTS") {
                        console.log(`Hi ${result.account.discordUsername}.`);
                        console.log("Requests code successfully injected.");
                        requests.urlCheck();
                    }
                } else {
        ...
        sizes: {
            ...
            checkSelected() {
                this.list.forEach((size) => {
                    if (size.className.includes(this.classValues.selected)) {
            ...
            loadSaved() {
                chrome.storage.local.get(["websites"], function (result) {
                    safe.product.sizes.select(result.websites.solebox.sizes);
                });
            },
            select(sizes) {
                    ...
                    if (!sizes.length > 0) {
                        ...
                        sizes.forEach((size) => {
                            ...
                            if (
                                ...
                                console.log("Specified size available!");
        ...
        payment() {
            ...
            document
                ...
                .addEventListener("click", function () {
                    setTimeout(clearInterval(retryPaymentButtonClick), 500);
...
const requests = {
    ...
    checkout: {
        ...
        shipping: {
            ...
            shipUUID: "",
            process() {
                requests.generateCSRF(requests.checkout.shipping.getAdressID);
            },
            getAdressID() {
                ...
                fetch(
                    "https://www.solebox.com/on/demandware.store/Sites-solebox-Site/de_DE/CheckoutShippingServices-SelectShippingMethod?format=ajax",
                    {
                        ...
                        referrer: "https://www.solebox.com/de_DE/checkout",
                        referrerPolicy: "strict-origin-when-cross-origin",
                    ...
                    fetch(
                        "https://www.solebox.com/on/demandware.store/Sites-solebox-Site/de_DE/CheckoutShippingServices-ShippingRates?format=ajax",
                        {
                            ...
                            body: `selected=true&id=${
                                ...
                                result.websites.solebox.profile.adressType
                            }&snipesStore=&hermesId&postOfficeNumber=&packstationNumber=&postNumber=&postalCode=${
                                result.websites.solebox.profile.zipCode
                            }&countryCode=${
                                result.websites.solebox.profile.countryCode
                            }&carrierName=&suite=${
                                result.websites.solebox.profile.streetNumber
                                ...
                                " ",
                                "+"
                            )}&address2=${result.websites.solebox.profile.adress2.replaceAll(
                                " ",
                                "+"
                            )}&lastName=${
                ...
                chrome.storage.local.get(["websites"], function (result) {
                    fetch(
                        ...
                        if (response.status === 200) {
                            console.log("Shipping submited successfully.");
